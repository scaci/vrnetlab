...
class OpenWRT_vm(vrnetlab.VM):
    def __init__(
        ...
        disk_image = None
        for e in os.listdir("/"):
            if is_openwrt_base_image(e):
                disk_image = "/" + e
                break
        if disk_image is None:
            raise RuntimeError("No OpenWrt base image found")
    ...
    def bootstrap_spin(self):
        ...
        for filename in os.listdir("/"):
            if is_openwrt_base_image(filename):
                base_image = filename
    ...
    def _mgmt_ipv6_or_none(self):
        ...
        try:
            ...
            self.logger.warning("Ignoring invalid MGMT IPv6 address: %s", value)
            return None

    def _write_mgmt_ipv6_config(self, enabled):
        mgmt_ipv6 = self._mgmt_ipv6_or_none() if enabled else None
        if mgmt_ipv6:
            self.tn.write(
                f"uci set network.mgmt.ip6addr='{mgmt_ipv6}'\n".encode("utf-8")
            )
            time.sleep(0.5)
            self.tn.write(b"uci set network.mgmt.delegate='0'\n")
            time.sleep(0.5)
            return
        self.tn.write(b"uci -q del network.mgmt.ip6addr\n")
        time.sleep(0.5)
        self.tn.write(b"uci -q del network.mgmt.delegate\n")
        time.sleep(0.5)
        ...
        if "config interface 'mgmt'" not in output:
            ...
            if current_mgmt_address_ipv4 == expected_mgmt_address_ipv4:
                ...
                if self.mgmt_passthrough_ipv4_address != self.mgmt_address_ipv4:
                    # MGMT passthrough inactive; remove stale IPv6 config.
                    self.tn.write(b"uci -q del network.mgmt.ip6addr\n")
                    time.sleep(0.5)
                self.logger.warning(
